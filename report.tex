...
\section{Analysis of incorporated Blogs Posts}
After the integration of the pipeline into Pollux (see \autoref{appendix:website} and \citet{czolkoss-hettwerPolitikwissenschaftlicheBlogsSichtbar2023} for the website layout), we analyze the incorporated Blogs. Integration started in July 2023 and we analyze Blog entries until October 2023. We analyze the topics of the blogs entries by fitting a topic model on the \texttt{summary} field of the entries. Before, the fitting we translated all summaries into English to make the topic model, as we want to focus on the areas the blogs cover as opposed to language difference. We additionally use the \texttt{publication date} field to visualize topics over time.
